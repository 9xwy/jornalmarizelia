...
drop policy if exists "public read jornal media" on storage.objects;
create policy "public read jornal media"
on storage.objects
for select
to anon, authenticated
using (bucket_id = 'jornal-media');

drop policy if exists "admin upload jornal media" on storage.objects;
create policy "admin upload jornal media"
on storage.objects
for insert
to authenticated
with check (bucket_id = 'jornal-media' and public.is_allowed_admin());

drop policy if exists "admin update jornal media" on storage.objects;
create policy "admin update jornal media"
on storage.objects
for update
to authenticated
using (bucket_id = 'jornal-media' and public.is_allowed_admin())
with check (bucket_id = 'jornal-media' and public.is_allowed_admin());

drop policy if exists "admin delete jornal media" on storage.objects;
create policy "admin delete jornal media"
on storage.objects
for delete
to authenticated
using (bucket_id = 'jornal-media' and public.is_allowed_admin());
